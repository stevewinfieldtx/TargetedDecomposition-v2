"""
TrueWriting Online — CPP Generator for Public Content
=====================================================
Ported from TrueWriting's analyzer.py for use inside TDE.
Analyzes YouTube transcripts, web content, etc. to generate
a Communication Personality Profile (CPP / TW-0).

Usage (subprocess from Node.js):
  echo '{"segments": [...]}' | python truewriting_online.py

Input JSON:
  { "segments": [{"text": "...", "source_id": "...", "title": "...", "date": "..."}] }

Output: TW-0 profile JSON to stdout.
"""
...
# Install deps if missing (Railway container)
try:
    import textstat
except ImportError:
    os.system('pip install --break-system-packages -q textstat 2>/dev/null')
    import textstat
...
try:
    ...
    os.system('pip install --break-system-packages -q nltk 2>/dev/null')
    import nltk
    from nltk.tokenize import sent_tokenize, word_tokenize
    from nltk.corpus import stopwords
...
class ContentMessage:
    """Universal message container for transcript segments."""
    def __init__(self, body='', subject='', to_addresses=None, date=None,
                 message_id=None, source_id='', source_type='transcript',
                 speaker=None, title=''):
        self.body = body
...
class TranscriptIngester:
    ...
    def from_texts(cls, texts, source_ids=None, titles=None, dates=None, speakers=None):
        ...
        for i, text in enumerate(texts):
            ...
            if dates and i < len(dates) and dates[i]:
                try:
                    date = datetime.fromisoformat(str(dates[i]).replace('Z', '+00:00'))
                except:
                    pass
            messages.append(ContentMessage(
                body=text,
                source_id=source_ids[i] if source_ids and i < len(source_ids) else f'seg_{i}',
    ...
    def from_json(cls, data):
        ...
        return cls.from_texts(
            ...
            source_ids=[s.get('source_id', '') for s in segments],
            titles=[s.get('title', '') for s in segments],
            dates=[s.get('date') for s in segments],
    ...
    def _clean_transcript(text):
        text = re.sub(r'\[.*?\]', '', text)
        text = re.sub(r'\(.*?\)', '', text)
        text = re.sub(r'\b(um|uh|er|ah|like,)\b', '', text, flags=re.I)
...
# ============================================================
#  CLI ENTRY POINT (called from Node.js subprocess)
# ============================================================
...
if __name__ == '__main__':
    ...
    # Ingest segments
    messages = TranscriptIngester.from_json(input_data)

    if len(messages) < 3:
        print(json.dumps({"error": f"Need at least 3 segments with 10+ words (got {len(messages)})"}))
        sys.exit(1)

    # Clean transcripts
    for msg in messages:
        msg.body = TranscriptIngester._clean_transcript(msg.body)
