...
const { submitBatch, batchAvailable } = require('../utils/anthropic-batch');
...
function buildDeepFillContext(solutionUrl, solutionName, msip) {
  return 'SOLUTION: ' + (solutionName || msip.product_name || 'Unknown') +
...
const DEEP_FILL_SYSTEM = 'You are a thorough market research analyst. Provide detailed, factual research. Never fabricate data.';

function buildTopicPrompt(topic, context) {
  return topic.prompt + '\n\n' + context +
    '\n\nProvide detailed, specific findings. If you cannot find information on a sub-topic, skip it rather than making things up.';
}

async function ingestDeepFillResult(engine, collectionId, solutionUrl, solutionName, msip, topicId, text) {
  return engine.ingest(collectionId, 'text', text, {
    title: (solutionName || msip.product_name || 'Solution') + ' — ' + topicId.replace(/_/g, ' '),
    context: 'Deep research on ' + topicId + ' for ' + (solutionName || solutionUrl),
  });
}

async function runDeepFill(engine, collectionId, solutionUrl, solutionName, msip) {
  var context = buildDeepFillContext(solutionUrl, solutionName, msip);

  // Prefer Anthropic Batch API when configured — 50% discount, non-blocking, Claude quality.
  // Falls back to sequential OpenRouter free-model path if batch unavailable or fails.
  if (batchAvailable()) {
    try {
      await runDeepFillBatch(engine, collectionId, solutionUrl, solutionName, msip, context);
      return;
    } catch (err) {
      console.log('  [Deep Fill] Batch path failed (' + err.message + '), falling back to OpenRouter sequential');
    }
  }

  await runDeepFillSequential(engine, collectionId, solutionUrl, solutionName, msip, context);
}

async function runDeepFillBatch(engine, collectionId, solutionUrl, solutionName, msip, context) {
  console.log('\n  [Deep Fill] Submitting ' + DEEP_FILL_TOPICS.length + ' topics to Anthropic Batch API...');

  var requests = DEEP_FILL_TOPICS.map(function(topic) {
    return {
      custom_id:   topic.id,
      prompt:      buildTopicPrompt(topic, context),
      system:      DEEP_FILL_SYSTEM,
      maxTokens:   4000,
      temperature: 0.3,
    };
  });

  var results = await submitBatch(requests);
  ...
  for (var i = 0; i < DEEP_FILL_TOPICS.length; i++) {
    ...
    var entry = results.get(topic.id);

    if (!entry) {
      console.log('  [Deep Fill] ' + topic.id + ': no result returned');
      skipped++;
      continue;
    }

    if (entry.error) {
      console.log('  [Deep Fill] ' + topic.id + ': batch error — ' + entry.error);
      errored++;
      continue;
    }
    ...
    try {
      await ingestDeepFillResult(engine, collectionId, solutionUrl, solutionName, msip, topic.id, text);
      console.log('  [Deep Fill] ' + topic.id + ': ingested');
      ingested++;
    } catch (err) {
      console.log('  [Deep Fill] ' + topic.id + ': ingest error — ' + err.message);
      errored++;
    }
  }

  console.log('  [Deep Fill] Batch complete for ' + collectionId + ' — ingested=' + ingested + ' skipped=' + skipped + ' errored=' + errored);
}

async function runDeepFillSequential(engine, collectionId, solutionUrl, solutionName, msip, context) {
  var FREE_MODEL = 'openrouter/free';

  console.log('\n  [Deep Fill] Starting background research (' + DEEP_FILL_TOPICS.length + ' topics, free models via OpenRouter)...');
  ...
  for (var i = 0; i < DEEP_FILL_TOPICS.length; i++) {
    ...
    try {
      var prompt = buildTopicPrompt(topic, context);
      ...
      if (result && result.length > 50) {
        await ingestDeepFillResult(engine, collectionId, solutionUrl, solutionName, msip, topic.id, result);
        console.log('  [Deep Fill] ' + topic.id + ': ingested');
      } else {
        console.log('  [Deep Fill] ' + topic.id + ': no useful content returned');
      }
